...
def trend_picture_generate(record):
    iterations = np.arange(record.shape[0])  # 生成迭代次数的数组
    # 绘制散点图，每个迭代的三个点用不同颜色表示（可选）
    colors = ['r', 'g', 'b']  # 例如，使用红色、绿色和蓝色
    plt.figure(figsize=(10, 6))  # 设置图形大小
    for i in range(record.shape[1]):
        plt.scatter(iterations, record[:, i], color=colors[i], label=f'Evaluation {i + 1}')
        # 设置图形标题和标签
    plt.title('GWO-Optimization-Trend')
...
def optimize():
    ...
    max_population = 500
    iteration = 15
    people = np.zeros((max_population, 16))  # 初始化一个全零数组
    people_best = np.zeros(16)
    record = np.zeros((iteration, 3))
    # 遍历每一列，并生成随机值
    ...
    for iterations in range(iteration):
        ...
        ID = find_leading_wolf(eva)
        for _ in range(3):
            record[iterations][_] = eva[ID[_]]
        for i in range(len(ID)):
            Gemo_Generate.CST_airfoil_file_generate(people[ID[i]], paths.tip_file, paths.mid_file, paths.root_file)
            vsp_4.create_Geom_4(paths.tip_file, paths.mid_file, paths.root_file, angle=people[ID[i]][15])
            # CMy, cl
            x_cg = 0.0287 + 0.0119 * people[ID[i]][15]  # 给定设定的重心位置
            angle = people[ID[i]][15] * np.pi / 180
        ...
        print(eva[ID[0]])
    trend_picture_generate(record)
    Gemo_Generate.CST_airfoil_file_generate(people_best, paths.tip_file, paths.mid_file, paths.root_file)
    vsp_4.create_Geom_4(paths.tip_file, paths.mid_file, paths.root_file, angle=people_best[15])
...
# Gemo_Generate.CST_airfoil_file_generate(
#     np.array([0.15191177, 0.15191177, 0.15191177, 0.15191177, 0.33064895, -0.33573143,
#               -0.33441193, 0.15191177, 0.15191177, 0.15191177, 0.15191177, -0.32585748,
#               -0.33314033, -0.30403477, 0.20675072, 10.12745129]), paths.tip_file,
