...
def steady_velocity_evaluation(cl, cd):
    ...
    rho = 1.225  # 空气密度
    S = 0.78486  # 主翼面积
    v_ctrl = np.sqrt((2 * g * mass) / (rho * S * cl)) - 1e-3  # 平飞配平 初始值预估
    # 从matlab中迁移的函数
...
# plt.show()
#
# plt.imshow(velocity_record, cmap='viridis', extent=[cl_start, cl_end, cd_start, cd_end], aspect='auto', origin='lower',
#            interpolation='bicubic')
